/**
 * Track C – Sprachqualitäts-Scanner
 ...
 * C-3: nspell Deutsches Wörterbuch (via dynamic import – dictionary-de v3 ESM)
...
const BWR_WHITELIST = new Set([
  ...
  // App-Begriffe
  'BuchungsWerk','BuchungsEngine','LumiTec','AlpenTextil','VitaSport','Waldform',
  'GmbH','KG','OHG','sensei',
  ...
  'Realschule','Realschulen','Bayern','Jahrgangsstufen','LehrplanPLUS',
  'Satzeinstieg','Satzkonstruktion','Wortreihenfolge','Signalwörter',
  // BwR Fachwörter (häufig im Pool, nspell kennt Komposita nicht immer)
  'Buchwert','Jahresende','Anfangsbestand','Schlussbestand','Nutzungsdauer',
  'Nettoausfall','Zahlungsziel','Endbestand','Jahresgewinn','Warenwert',
  'Bankguthaben','Einstandswert','Folgejahr','Privatentnahmen','Privateinlagen',
  'Betriebsergebnis','Deckungsbeitrag','Schlussbilanz','Istbestand','Aufwandskonto',
  'Vorjahr','Kontoauszug','Überweisung','Diagrammart','Anführungszeichen',
  'Restlaufzeit','Tilgungsplan','Annuität','Deckungsbeitragsrechnung',
  'Kostenträger','Kostenstellen','Kostenarten','Zuschlagskalkulation',
  'Maschinenstundensatz','Vollkostenrechnung','Teilkostenrechnung',
  'Deckungsbeiträge','Gewinnschwelle','Beitragssatz','Lohnsteuerkarte',
  'Umsatzsteuervoranmeldung','Voranmeldung','Kleinunternehmer',
  'Abschreibungsmethode','Lineare','Degressive','Leistungsabschreibung',
  // Häufige deutsche Wörter die nspell (Hunspell-Anbindung) möglicherweise nicht kennt
  'bilden','beurteilen','betrug','senden','erfassen','anteil','kurzfr',
  'Jahresabschluss','Jahresabschlüsse','Periodenabgrenzung',
  // Zusammengesetzte Wörter (Hunspell-DE kennt viele Komposita nicht)
  'Überschrift','Rücksendung','gewähren','Bestandsveränderung',
  'Vorabschlussbuchung','Banküberweisung','Kurswert','Insolvenzverfahren',
  'Amtsgericht','auflösen','Rückstellung','Rückstellungen','ZielVP',
  'Aktivkonto','Passivkonto','Ertragskonto','Aufwandskonto',
  'Debitorenkonto','Kreditorkonto','Abschlussbuchung','Eröffnungsbuchung',
  'Jahresfehlbetrag','Jahresüberschuss','Eigenkapitalveränderung',
  'Geschäftsjahr','Wirtschaftsjahr','Bilanzstichtag','Bewertungsmaßstab',
  'Niederstwertprinzip','Imparitätsprinzip','Realisationsprinzip',
  'Mahngebühren','Mahnschreiben','Säumniszuschlag','Delkredere',
  'Wertpapierbestand','Aktiendepot','Dividendenzahlung','Nennwert',
  'Anschaffungswert','Teilwertabschreibung','Zuschreibung',
]);

async function loadSpellChecker() {
  try {
    // dictionary-de v3 is ESM with top-level await – must use dynamic import()
    const { default: nspell } = await import('nspell');
    const { default: deDic }  = await import('dictionary-de');
    // deDic = { aff: Uint8Array, dic: Uint8Array }
    return nspell(deDic);
  } catch {
    return null;
  }
...
async function runC3Spellcheck(files) {
  ...
  for (const file of files) {
    ...
    const snippets = extractGermanTemplates(content);
    for (const { text, line } of snippets) {
      // Pre-process: remove escape sequences (\n, \t etc.) and template expressions
      // to avoid Unicode-boundary false positives (ü/Ü are not \w → \b splits wrongly)
      const cleaned = text
        .replace(/\\[nrtvf\\'"]/g, ' ')  // strip \n \t \r etc.
        .replace(/\$\{[^}]+\}/g, ' ')
        .replace(/<[^>]{1,30}>/g, ' ');  // strip HTML tags like <strong>, <br>
      // Split on non-German-letter chars (avoids \b Unicode boundary issues)
      const words = cleaned
        .split(/[^A-ZÄÖÜa-zäöüß]+/)
        .filter(w => w.length >= 5);
      for (const word of words) {
        // Skip whitelisted and uppercase acronyms (all-caps abbreviations)
...
export async function runLanguageScanner({ quiet = false } = {}) {
  ...
  if (!quiet) process.stdout.write(`   ${icon} ${summary.totalPass} PASS · ${summary.totalWarn} WARN · ${summary.totalFail} FAIL\n`);
  return summary;
}

// ── Standalone-Modus (npm run test:digital:lang) ──────────────────────────────
...
if (isMain) {
  ...
  console.log('── Track C: Sprachqualitäts-Scanner ──');
  const result = await runLanguageScanner();
