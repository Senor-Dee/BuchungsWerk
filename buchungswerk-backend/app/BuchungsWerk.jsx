...
// ══════════════════════════════════════════════════════════════════════════════
const API_URL = "http://192.168.68.54:8000";  // ← ggf. IP eintragen
...
function SchrittTyp({ onWeiter, onBelegEditor, onEigeneBelege }) {
  ...
  const [pruefungsart, setPruefungsart] = useState(null);
  const [klasse, setKlasse] = useState(null);
  const [datum, setDatum] = useState(new Date().toISOString().split("T")[0]);
  const [punkteModus, setPunkteModus] = useState("anzahl"); // "anzahl" | "punkte"
  const [anzahl, setAnzahl] = useState(5);
  const [maxPunkte, setMaxPunkte] = useState(30);
  const [selectedThemen, setSelectedThemen] = useState({});
  const [expandedLBs, setExpandedLBs] = useState({});
  const [werkstoffId, setWerkstoffId] = useState("rohstoffe");
  const [komplexOpts, setKomplexOpts] = useState({
...
function ExportModal({ aufgaben, config, firma, kiHistorie, onSchliessen }) {
  ...
  // Word: HTML-Datei direkt herunterladen – Word/Pages öffnet .html nativ
  const exportWord = () => {
    try {
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: "10px", marginTop: "8px", flexWrap: "wrap" }}>
        <button onClick={onNeu} style={S.btnSecondary}>← Neue Aufgabe erstellen</button>
        <button onClick={() => {
          try {
            const ki = JSON.parse(localStorage.getItem("buchungswerk_ki_export") || "[]");
            setKiHistorie(ki);
  ...
  return (
    <div style={S.page}>
      ...
      <div style={S.topbar}>
        ...
        {/* Rechts: ISB-Info */}
        <div style={{ minWidth: 180, flexShrink: 0, textAlign: "right", fontSize: 10, color: "#334155", fontWeight: 600, letterSpacing: ".06em", textTransform: "uppercase" }}>
          ISB · Realschule · 2025
        </div>
      </div>

      <div style={S.container}>
        {schritt === 1 && <SchrittTyp onWeiter={cfg => { setConfig(cfg); setSchritt(2); }} onBelegEditor={() => setBelegEditorOffen(true)} onEigeneBelege={() => setEigeneBelegeOffen(true)} />}
        {schritt === 2 && <SchrittFirma config={config} onWeiter={f => { setFirma(f); setSchritt(3); }} onZurueck={() => setSchritt(1)} />}
        {schritt === 3 && <SchrittAufgaben config={config} firma={firma} onNeu={reset} />}
      </div>
